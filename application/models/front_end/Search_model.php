...
class Search_model extends CI_Model {
	...
	{	
		...
		if($this->input->post('photo_search') && $this->input->post('photo_search') !='')
		{
			...
			if(isset($photo_search) && $photo_search =='photo_search')
			{
				...
				$where_search[]= " ( 
					photo1 != '' and photo1_approve ='APPROVED' and ( photo_view_status = '1' $plna_status_where ) ) ";
							...
							if(isset($sender_data) && $sender_data!= ''){
								...
								if(isset($sender_data['photo1']) && $sender_data['photo1']!='' && $sender_data['photo1_approve']=='APPROVED' && file_exists($path_photos.$sender_data['photo1']))
								{
									...
									if(isset($sender_data['gender']) && $sender_data['gender'] == 'Male'){
										$defult_photo = $web_name.'assets/front_end/img/default-photo/male.gif';
									}else{
										$defult_photo = $web_name.'assets/front_end/img/default-photo/female.gif';
									}
								}
	...
	*/
	...
	public function member_like()
	{
		...
		$created_on = $this->common_model->getCurrentDate();//date('Y-m-d H:i:s');
				...
				if($remaining_contact > 0 && $plan_expired >= $today_date)
				{
					...
					$member_data = $this->common_model->get_count_data_manual('register_view',$where_array_member,1,'');
						...
						if($member_data['city_name'] != '' && $member_data['state_name'] != '' && $member_data['country_name'] != ''){
							$address = $member_data['city_name'].', '.$member_data['state_name'].', '.$member_data['country_name'];
						}else{
						...
						if($time_to_call==''){ $time_to_call='Not Available';}
						...
						if($member_data['contact_view_security'] == 1){
								...
								<div class='modal-dialog'>
									<div class='modal-header'>
										...
										<h4 class='modal-title'>
											<i class='fa fa-phone'></i> Contact Details of $from_id
										</h4>
									</div>
							...
							$data['contact_details']="
									<div class='modal-dialog'>
										<div class='modal-content'>
											<div class='modal-header'>
												<button type='button' class='close' data-dismiss='modal'>&times;</button>
												<h4 class='modal-title'><i class='fa fa-phone'></i> Contact Details of $from_id</h4>
											</div>
											<div class='alert alert-info'>
												<h4>
													$note
											...
											<div class='modal-body'>
												<div class='xxl-7 xl-7 l-7 m-16 xs-16 s-16'>
													<img src='$image' title='$from_id' alt='$from_id' style= 'height:200px; width:190px;'>
												</div>
							...
							if($expressinterest_count > 0){
								...
								if($block_count > 0){
									$data['contact_details']="<div class='modal-dialog'> <div class='modal-content'><div class='modal-header'> <button type='button' class='close' data-dismiss='modal'> &times;</button> <h4 class='modal-title'><i class='fa fa-phone'></i> Contact Details of $from_id</h4></div>  <div class='alert alert-info'> <h4>This member has blocked you. You can't see his contact details.</h4></div></div></div>";
								}
								else
								{
									if(isset($this->common_model->is_demo_mode) && $this->common_model->is_demo_mode == 1)
								...
								$data['contact_details']="<div class='modal-dialog'> <div class='modal-content'><div class='modal-header'> <button type='button' class='close' data-dismiss='modal'> &times;</button> <h4 class='modal-title'><i class='fa fa-phone'></i> Contact Details of $from_id</h4></div>  <div class='alert alert-info'> <h4>This member only shows his/her contact details if you have already sent him/her express interest, and he/she has accepted it.</h4></div></div></div>";
							}
					...
					if($contact_checker > 0 && $payments_data_count > 0 && $plan_expired >= $today_date){
						...
						if($member_data['contact_view_security'] == 1){
							...
							$data['contact_details']="<div class='modal-dialog'> <div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal'> &times;</button> <h4 class='modal-title'><i class='fa fa-phone'></i> Contact Details of $from_id</h4> </div> <div class='alert alert-info'><h4>Contact details have been already seen.</h4></div><div id='respond_contact_details'> </div> <div class='modal-body'> <img src='$image' title='$from_id' alt='$from_id' style= 'height:200px; width:190px;'> <span> <i class='fa fa-user ne_mrg_ri8_10'></i> Matri Id : $from_id</span><br/> <span> <i class='fa fa-user ne_mrg_ri8_10'></i> Name  : $username</span><br/> <span> <i class='fa fa-home ne_mrg_ri8_10'></i> Address : $address</span><br/> <span> <i class='fa fa-user ne_mrg_ri8_10'></i> Mobile : $mobile</span><br/> <span> <i class='fa fa-user ne_mrg_ri8_10'></i> Email Id : $email</span><br/> </div></div></div>";
						}else{
							$data['contact_details']="<div class='modal-dialog'><div class='modal-content'><div class='modal-header'> <button type='button' class='close' data-dismiss='modal'> &times;</button> <h4 class='modal-title'><i class='fa fa-phone'></i> Contact Details of $from_id</h4> </div> <div class='alert alert-info'><h4>Member not share his contact details.</h4> </div></div></div>";
						}
					}else{
						$data['contact_details']="<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal'> &times;</button><h4 class='modal-title'><i class='fa fa-phone'></i> Contact Details of $from_id</h4></div> <div class='alert alert-info'><h4>You have not enough contact remain, Please upgrade your plan.</h4></div></div></div>";
...
//$this->db->where('is_deleted','No');
			...
			if($query->num_rows() == 1)
			{
				...
				$photo_pswd_where = " ( ph_requester_id ='$my_id') and ( ph_receiver_id ='$member_id') and (status != '0') and ( receiver_response ='Rejected') ";
				$this->db->select('*');
				$this->db->where($photo_pswd_where);
	//$this->db->where('is_deleted','No');
				$this->db->limit(1);
