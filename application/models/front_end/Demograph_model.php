...
class Demograph_model extends CI_Model {
	...
	{	
		...
		$where_array_relegion = array('status'=>'APPROVED',"LOWER(TRIM(religion_name)) IN ('knanaya','latin','syro malabar','syro malankara')");
		...
		$where_array_not_relegion = array('status'=>'APPROVED',"LOWER(TRIM(religion_name)) NOT IN ('knanaya','latin','syro malabar','syro malankara')");
		...
		if($gender != '' || $religion != ''){
			...
			$where_array_all = $gender.$age_query." and status = 'APPROVED' and is_deleted='No' and (religion!='' and religion!='0')";
		}
