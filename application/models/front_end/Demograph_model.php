...
class Demograph_model extends CI_Model {
	...
	{	
		...
		/*$where_array = array('status'=>'APPROVED',"LOWER(TRIM(country_name)) IN ('india','australia','usa','u s a','united states','united states of america','canada','pakistan','uk','united kingdom')");
		$selected_country_data = $this->common_model->get_count_data_manual('country_master',$where_array,2,'');
		$selected_country_count_data = $this->common_model->get_count_data_manual('country_master',$where_array,0,'');
		
		$where_array1 = array('status'=>'APPROVED',"LOWER(TRIM(country_name)) NOT IN ('india','australia','usa','u s a','united states','united states of america','canada','pakistan','uk','united kingdom','united arab emirates','iran','israel','saudi arabia','egypt', 'qatar', 'oman', 'syria','iraq', 'kuwait')");
		$unselected_country_data = $this->common_model->get_count_data_manual('country_master',$where_array1,2,'');
		$unselected_country_count_data = $this->common_model->get_count_data_manual('country_master',$where_array1,0,'');
		
		$where_array2 = array('status'=>'APPROVED',"LOWER(TRIM(country_name)) IN ('united arab emirates','iran','israel','saudi arabia','egypt', 'qatar', 'oman', 'syria','iraq', 'kuwait')");
		$middle_east_country_data = $this->common_model->get_count_data_manual('country_master',$where_array2,2,'');
		$middle_east_country_count_data = $this->common_model->get_count_data_manual('country_master',$where_array2,0,'');
		
		$selected_countries = array();
		foreach($selected_country_data as $row_country){
			$selected_countries[strtolower(trim($row_country['country_name']))] = array('country_id'=>$row_country['id'],'country_code'=>$row_country['country_code'],'country_name'=>strtolower(trim($row_country['country_name'])),'status'=>$row_country['status']);
		} 
		
		$unselected_countries = array();
		foreach($unselected_country_data as $row_uncountry){
			$unselected_countries[] = $row_uncountry['id'];
		} 
		
		$middle_east_countries = array();
		foreach($middle_east_country_data as $row_middle_east){
			$middle_east_countries[] = $row_middle_east['id'];
		} 
		
		if($country_data == 'All'){
			$country = "";
		}else if($country_data == 'Gulf'){
			if($middle_east_country_count_data > 0){
					$middle_east_countries_comma = implode("','",$middle_east_countries);
					$country = " and country_id IN ('".$middle_east_countries_comma."') "; 
			}else{
				$country = "";
			}
		}else if($country_data == 'Others'){
			if($unselected_country_count_data > 0){
					$unselected_countries_comma = implode("','",$unselected_countries);
					$country = " and country_id IN ('".$unselected_countries_comma."') "; 
			}else{
				$country = "";
			}
		}else{
			$country_data = strtolower(trim($country_data));
			if($this->in_array_r($country_data,$selected_countries))
			{
				$country = " and country_id='".$selected_countries[$country_data]['country_id']."'";
			}else{
				$country = "";
			}
		}*/
		...
		//$country != '' || 
		if($gender != '' || $religion != ''){
			$where_array_all = $gender.$religion.$age_query." and status != 'Inactive'";
		}else{
			$where_array_all = $gender.$age_query." and status != 'Inactive'";
		}
