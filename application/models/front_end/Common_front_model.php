...
class Common_front_model extends CI_Model {
	...
	{
		/*,'Manglik'=>'manglik','Star'=>(($data['star']==null || $data['star']==''))?'star':'star_str','Horoscope'=>'horoscope','Gothra'=>'gothra','Moonsign'=>(($data['moonsign']==null || $data['moonsign']==''))?'moonsign':'moonsign_str'*/
					...
					foreach($vals as $keyss=>$valss)
					{
						...
						else
						{
							...
							if($category_id_key == 'location_info'){
								if($data['contact_view_security'] == '1'){
	...
	{
		/*,'Manglik'=>'manglik','Star'=>'star','Horoscope'=>'horoscope','Gothra'=>'gothra','Moonsign'=>'moonsign'*/
		/*,'Manglik'=>'part_manglik','Star'=>'part_star'*/
		$reset_opt_arrya = array(
			'fileds'=>array(
				 'basic_info'=>array(
                    'Basic Details'=>array('Name'=>'username','Marital Status'=>'marital_status','Mother Tongue'=>'mtongue_name','Languages Known'=>'languages_known','Total Children'=>'total_children','Status Children'=>'status_children','Height'=>'height','Weight'=>'weight','Birthdate'=>'birthdate')
                ),
				...
				foreach($val as $category_label_key=>$vals)
				{
					...
					foreach($vals as $keyss=>$valss)
					{
						if($category_id_key == 'photo_info')
						{
							...
							$label_value[] = array('title'=>$keyss,'value'=>$str_val,'string_id'=>($keyss == 'Age Preference' || $keyss == 'Height Preference')? $valss:(($data[$valss]==null)?'':((isset($valss) && $valss=='birthdate')?date("d-m-Y", strtotime($data[$valss])):$data[$valss])),'id'=>($keyss == 'Age Preference')?'part_age':(($keyss == 'Height Preference')?'part_height':$valss));
						}
						/*else
						{
	...
	public function field_percentage_array()
	{
		...
		//Social Religion Attributes as 5% value
		'religion'=>'2',
		'caste'=>'2',
		'subcaste'=>'1',
		/*'gothra'=>'1',
		...
		'no_of_married_sister'=>'1',
		 ...
		 //Partner Preference as 10% value
		 'looking_for'=>'1',
		 'part_frm_age'=>'1',
		 'part_to_age'=>'1',
		 'part_expect'=>'1',
