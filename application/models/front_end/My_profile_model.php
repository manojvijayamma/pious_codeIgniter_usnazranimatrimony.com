...
class My_profile_model extends CI_Model {
	...
	function i_viewed_profile($post=0,$page='')
	{
		...
		$where_arra=array('who_viewed_my_profile.my_id'=>$login_user_matri_id,"register_view.is_deleted"=>'No');
	...
	function photo_pass_request_received($post=0,$page='')
	{
		...
		//$login_user_matri_id = $this->common_front_model->get_session_data('matri_id');
		$where_arra=array('ph_receiver_id'=>$login_user_matri_id,'rec_delete' => 'No');
		$this->db->join('register','photoprotect_request.ph_requester_id = register.matri_id');
		if($post == 0){
			$data = $this->common_model->get_count_data_manual('photoprotect_request',$where_arra,0,'','ph_reqid desc','','','','');
		}else{
			$data = $this->common_model->get_count_data_manual('photoprotect_request',$where_arra,2,'photoprotect_request.ph_reqid,photoprotect_request.ph_requester_id,photoprotect_request.ph_receiver_id,photoprotect_request.ph_msg,photoprotect_request.ph_reqdate,photoprotect_request.receiver_response,photoprotect_request.status,photoprotect_request.rec_delete,photoprotect_request.sen_delete,register.id as user_id','ph_reqid desc',$page,'','','','');
		}
	...
	function send_photo_pass()
	{
		...
		if($check_count<=0){
			//$data_register_login = $this->common_model->get_count_data_manual('register','matri_id = "'.$login_user_matri_id.'"',1,'photo_password');
			$data_receiver = $this->common_model->get_count_data_manual('register','matri_id = "'.$to_matri_id.'"',1,'username,email');
			
		//if(isset($data_register_login) && $data_register_login !='' && count($data_register_login) > 0 && isset($data_receiver) && $data_receiver !='' && count($data_receiver) > 0 )
		if(isset($data_receiver) && $data_receiver !='' && count($data_receiver) > 0 )
		{
			$username = $this->common_front_model->get_session_data('username');
			//$photo_pass = $data_register_login['photo_password'];
	...
	function photo_pass_request_sent($post=0,$page='')
	{
		...
		//$login_user_matri_id = $this->common_front_model->get_session_data('matri_id');
		$where_arra=array('ph_requester_id'=>$login_user_matri_id,'sen_delete' => 'No');
		
		if($post == 0){	
			$this->db->join('register','photoprotect_request.ph_receiver_id = register.matri_id');
			$data = $this->common_model->get_count_data_manual('photoprotect_request',$where_arra,0,'','','','','','');
		}else{
			$this->db->join('register','photoprotect_request.ph_receiver_id = register.matri_id');
			$data = $this->common_model->get_count_data_manual('photoprotect_request',$where_arra,2,'photoprotect_request.ph_reqid,photoprotect_request.ph_requester_id,photoprotect_request.ph_receiver_id,photoprotect_request.ph_msg,photoprotect_request.ph_reqdate,photoprotect_request.receiver_response,photoprotect_request.status,photoprotect_request.rec_delete,photoprotect_request.sen_delete,register.id as user_id','',$page,'','','','');
	...
	function like_profile($post=0,$page='')
	{
		...
		$where_arra=array('member_likes.is_deleted'=>'No','member_likes.like_status'=>'Yes','member_likes.my_id'=>$login_user_matri_id);
	...
	public function common_delete_list_all_profile()
	{
		...
		if($matri_id !='' && $page_name !='' )
		{
			...
			}elseif(isset($page_name) && $page_name == 'I Viewed Profiles'){
