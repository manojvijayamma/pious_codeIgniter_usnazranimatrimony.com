                    ...
                    <?php 
						...
						foreach($field_array as $fields=>$val_arr)
                        {
							...
							foreach($val_arr as $key_c=>$val_per)
							{
								...
								if(isset($login_user_data[$key_c]) && (trim($login_user_data[$key_c]) =='' || ($login_user_data[$key_c] =='0')) || $login_user_data[$key_c] == NULL || ($key_c == 'cpass_status' && $login_user_data['cpass_status']=='Not-Verify') || ($key_c=='mobile_verify_status' && $login_user_data['mobile_verify_status']=='No'))
								{
					...
					?>
                    	<div class="item <?php echo $first_tab_active; ?>">
                            ...
                            <div class="xxl-13 xl-13 xs-16 m-16 s-16 l-13">
                            	 <?php if($fields == 'cpass-status'){?>
                                 	<p class="bold small">Verify Your Email Id. <span class="text-green">(+<?php echo $point_plus; ?>%)</span></p>
								<?php }elseif($fields == 'mobile-verify-status'){?>
                                	<p class="bold small">Verify Your Mobile Number. <span class="text-green">(+<?php echo $point_plus; ?>%)</span></p>
								<?php }else{?>
								 ...
								 if(in_array($fields,array('cover-photo','horoscope-photo','photo','id-proof','cpass-status','mobile-verify-status'))){
                                    ...
                                    <?php }
									else if($fields == 'cpass-status'){?>
                                     	<a href="javascript:void(0);" data-toggle="modal" data-target="#myModal_verify_email" id="myModal_verify_email_btn">Click to Verify Your Email Id</a>
                               		<?php }
									else if($fields == 'mobile-verify-status'){?>
                                     	<a href="javascript:void(0);" data-toggle="modal" data-target="#myModal_verify_mobile" id="myModal_verify_mobile_btn">Click to Verify Your Mobile Number</a>
                                	<?php }
								}
								else{
