...
class Lead_generation extends CI_Controller {
	...
	public function lead_generation_list($status ='ALL', $page =1,$clear_search='no')
	{
		...
		$this->lead_generation_model->lead_gen_list($status,$page,'');
	}
	public function lead_generation_report($status ='ALL', $page =1, $clear_filter='no'){
		...
		$this->common_model->labelArr =  array('email'=>'Lead Generation Email Id','next_followup_date'=>'Followup Date','assign_to_staff'=>'Staff Name','assign_to_franchise'=>'Franchise Name');
		$other_config = array(
			'addAllow'=>'no',
			'editAllow'=>'no',
			'default_order'=>'asc',
		...
		$this->common_model->display_selected_field = array('interest','lead_generation_id','email','assign_to_staff','staff_email_id','assign_to_franchise','franchise_email_id','next_followup_date');
