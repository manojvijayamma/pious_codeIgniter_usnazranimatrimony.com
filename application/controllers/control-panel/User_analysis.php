...
class User_analysis extends CI_Controller {
	...
	public function user_analysis_list($status ='ALL', $page =1, $clear_ip_filter='no')
	{
		...
		$this->common_model->data_tabel_filedIgnore = array('ip','address','country','id','latitude','longitude','postal','broadband_name','api_info','is_deleted');
		$this->common_model->data_tabel_filed = array('ip','address','country','total_count');
		$this->user_analysis_model->display_search_form();
		$this->common_model->common_rander('user_analysis_view', $status, $page , 'User Analysis Report','','visit_time',0,$other_config,'');
	...
	public function visit_links($ip='', $page =1,$clear_ip_filter='no'){
		...
		$this->common_model->created_on_fild = 'visit_time';
		$this->common_model->data_tabel_filedIgnore = array('id','latitude','longitude','postal','broadband_name','api_info','is_deleted');
		$this->user_analysis_model->display_search_form();
