...
class Message extends CI_Controller {
	...
	{	
		...
		if($is_ajax == 0)
		{
			//$this->common_model->extra_css_fr= array('css/BootSideMenu.css');
	...
	public function get_message_list($page=1)
	{
		...
		if(isset($message_list_count) && $message_list_count!='' && $message_list_count > 0 && isset($message_list) && $message_list!='')
		{
			...
			//$message = $message_list['sent'];
			
			//print_r($message_list[]);
			//exit;
			// $message['photo']='fedfgv';
			
			
			//print_r($this->common_front_model->photo_process($message_list['receiver']));exit;
	...
	{
		$this->common_model->display_top_menu_perm = 'No';
		$this->common_model->extra_css_fr= array('css/editor.css','css/BootSideMenu.css','css/select2.min.css');
		$this->common_model->extra_js_fr= array ('js/BootSideMenu.js','js/chosen.jquery.js','js/select2.min.js','js/editor.js');
		$base_url = $this->data['base_url'];
		$this->data['msg_id'] = $msg_id;
		$this->data['mode'] = $mode;
		$this->common_model->front_load_header();
		$this->load->view('front_end/compose_msg',$this->data);
		$this->common_model->front_load_footer();
	}
	...
	{	
		...
		$this->common_model->display_top_menu_perm = 'No';
		$this->common_model->extra_css_fr= array('css/BootSideMenu.css','css/w3.css');
		$this->common_model->front_load_header();
		$this->data['msg_id'] = $msg_id;
		$this->data['mode'] = $mode;
