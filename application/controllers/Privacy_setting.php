...
class Privacy_setting extends CI_Controller {
	...
	{	
		...
		$this->common_model->extra_css_fr= array('css/hover-img.css','css/bootstrap-touch-slider.css','css/BootSideMenu.css','css/chosen.css');
		$this->common_model->extra_js_fr= array('js/BootSideMenu.js','js/chosen.jquery.js','js/additional-methods.min.js');
	...
	public function block_profile()
	{
		...
		else
		{
			...
			if($user_agent!='NI-WEB')
			{
				...
				$data1['errormessage'] = $response;
				$data1['errmessage'] = $response;
				print_r($response);exit;
				$block_profile_data_count = $this->my_profile_model->block_list_profile(0);
				$block_profile_data = $this->my_profile_model->block_list_profile(1,1);
				
				$data1['continue_request'] = TRUE;
				$data1['tocken'] = $this->security->get_csrf_hash();
				$data1['status'] = 'success';
				$data1['total_count'] = $block_profile_data_count;
				if(isset($block_profile_data) && $block_profile_data!='')
				{
